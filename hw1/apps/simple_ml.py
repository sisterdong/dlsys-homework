...
def parse_mnist(image_filesname, label_filename):
    ...
    with gzip.open(image_filesname, "rb") as f:
        X = np.frombuffer(f.read(), dtype=np.uint8, offset=16).astype(np.float32)
        X = X/255.0
...
def softmax_loss(Z: ndl.Tensor, y_one_hot: ndl.Tensor):
    ...
    """
    return (log(exp(Z).sum(1)) - (Z * y_one_hot).sum(1)).sum() / Z.shape[0]
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, int(n/batch)):
        bx = ndl.Tensor(X[i*batch:(i+1)*batch])
        by = y[i*batch:(i+1)*batch]
        z = ndl.ops.relu(bx @ W1) @ W2
        by_one_hot = np.zeros((batch, n_class))
